...
* **`move (Distance:TUR_Location):Graphic`**<br>moves the turtle (starting at its current position) for `Distance` units without drawing. Positive values for `Distance` move forward, negative ones move backward - always keeping the original orientation. This method may be invoked outside an active path

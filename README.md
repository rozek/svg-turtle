...
* **`drawTo (x:TUR_Location, y:TUR_Location):Graphic`**<br>moves the turtle "aboslutely" (i.e., measured from the origin) to the given position (keeping its current orientation) drawing a straight line. If invoked outside an active path, a new path with the current turtle position, orientation and line style is started

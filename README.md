...
* **`move (Distance:TUR_Location):Graphic`**<br>*(moves the turtle relatively in the current direction for Distance units without drawing, positive distances move forward, negative distances move backward keeping the original orientation, may be invoked outside an active path)*
* **`moveTo (x:TUR_Location, y:TUR_Location):Graphic`**<br>*(moves the turtle aboslutely to the given position without drawing and keeping its current orientation, may be invoked outside an active path)*

...
* **`moveTo (x:TUR_Location, y:TUR_Location):Graphic`**<br>moves the turtle "absolutely" (i.e., relative to the origin) to the given position (keeping its current orientation) without drawing. This method may be invoked outside an active path
* **`draw (Distance:TUR_Location):Graphic`**<br>moves the turtle "relatively" (i.e., starting from its current position) in the current direction for `Distance` units drawing a straight line. Positive distances move forward, negative ones move backward. If invoked outside an active path, a new path with the current turtle position, orientation and line style is started
